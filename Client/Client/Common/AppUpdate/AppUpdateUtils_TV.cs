...
namespace Client.Common
{
    ...
    /// 
    /// V 1.0.0 2019-9-16 15:29:04
    /// 从 AppUpdateUtils 拷贝过来
    /// 由于测试在飞利浦电视中使用 Acr.UserDialogs.UserDialogs 弹窗控件无法用遥控进行左右切换操作
    /// 故 Acr.UserDialogs.UserDialogs 相关需要选择 确认的控件全部用回 Xamarin.Forms 原生的控件
    /// </summary>
    public class AppUpdateUtils_TV
    {
        Xamarin.Forms.ContentPage mPage;

        public AppUpdateUtils_TV(Xamarin.Forms.ContentPage page)
        {
            mPage = page;
        }
            ...
            (
                ...
                handle: async (soapResult) =>
                {
                    ...
                    else
                    {
                        ...
                        // 询问用户是否更新
                        bool r1 = await mPage.DisplayAlert
                        (
                            title: "确认更新",
